...
Vector2d Renderer::realToScreen(const Vector2d& pos){
    ...
    float x = (pos.x - cameraPos.x) * Config::scale + (w / 2.0f);
    float y = (h / 2.0f) - (pos.y - cameraPos.y) * Config::scale;
...
Vector2d Renderer::screenToReal(const Vector2d& pos){
    ...
    float x = ((pos.x - (w / 2.0f)) / Config::scale) + cameraPos.x;
    float y = (((h / 2.0f) - pos.y) / Config::scale) + cameraPos.y;
